...
int n;//点的数量
int m;//操作的数量
int p[N];//存储的是每个元素的父节点是谁
int sizeo[N];//表示每一个集合的大小，每一个集合中点的数量
//初始的时候，每个元素是一个单独的集合，每一个集合里只有单独的一个点，这个点的树根就是自己
//关于如何维护size，其实只有根节点的size是有意义的 ，也只需要保证根节点的size是有意义的

//最核心的就是这个find操作，采用了递归的方式
int find(int x)//返回x所在集合的编号，返回x的根节点（注意，不是父节点）+路径压缩
{
